...
| Empty `content` (None) but tokens consumed; `finish_reason: length` | Reasoning/"thinking" models (e.g. Qwen 3.x) spend the whole `max_tokens` budget on hidden reasoning before producing the final answer | Disable thinking mode: pass `extra_body={"chat_template_kwargs": {"enable_thinking": False}}` (OpenAI SDK) or `"chat_template_kwargs":{"enable_thinking":false}` (raw JSON). Alternatively raise `max_tokens` substantially so reasoning + answer both fit. |
